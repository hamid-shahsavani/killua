...
```ts showLineNumbers filename="components/components.tsx" copy
...
```ts showLineNumbers filename="utils/get-counter.ts" copy
import { counterSlice } from '@/slices/counter';
...
```ts showLineNumbers filename="utils/set-counter.ts" copy
import { counterSlice } from '@/slices/counter';
